...
async function fetchGame(
  ...
  try {
    // Build API URL
    let url = `${API_BASE}/api/games?page=${page}&page_size=${page_size}`;
    ...
    async function setGamingSort({ platforms, genre, developers, tags }, data) {
      ...
      try {
        if (platforms) {
          ...
          let devName = null;
          try {
            const res = await fetch(`${API_BASE}/api/developers/${developers}`);
            if (res.ok) {
          ...
          if (!tagName) {
            try {
              const res = await fetch(`${API_BASE}/api/tags/${tags}`);
              if (res.ok) {
